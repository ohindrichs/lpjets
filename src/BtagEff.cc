...
void BtagEff::Fill(Permutation& per, float thenvtx, bool thadcorrect, bool tlepcorrect, double theweight)
{
	...
	{
		test = 1;
		j[0] = per.BHad()->Px(); j[1] = per.BHad()->Py(); j[2] = per.BHad()->Pz(); j[3] = per.BHad()->E(); j[4] = bhad->csvIncl(); j[5] = bhad->CombinedMVA(); j[6] = bhad->CvsB();
		btagtree->Fill();
	...
	{
		test = 2;
		j[0] = per.BLep()->Px(); j[1] = per.BLep()->Py(); j[2] = per.BLep()->Pz(); j[3] = per.BLep()->E(); j[4] = blep->csvIncl(); j[5] = blep->CombinedMVA(); j[6] = blep->CvsB();
		btagtree->Fill();
	}
	...
	{
		test = 3;
		j[0] = per.WJa()->Px(); j[1] = per.WJa()->Py(); j[2] = per.WJa()->Pz(); j[3] = per.WJa()->E(); j[4] = wja->csvIncl(); j[5] = wja->CombinedMVA(); j[6] = wja->CvsB();
		btagtree->Fill();
		j[0] = per.WJb()->Px(); j[1] = per.WJb()->Py(); j[2] = per.WJb()->Pz(); j[3] = per.WJb()->E(); j[4] = wjb->csvIncl(); j[5] = wjb->CombinedMVA(); j[6] = wjb->CvsB();
		btagtree->Fill();
	}
